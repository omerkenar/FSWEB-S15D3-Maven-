...
public class WordCounter {
    public static Map<String, Integer> calculatedWord(){
        ...
        String[] splitedWorlds = text.split("\\s+");
        Map<String,Integer> counter = new HashMap<>();

        for(String splitedWorld:splitedWorlds){
            splitedWorld = splitedWorld.toLowerCase().replaceAll("[.,!?\"]", "");
            counter.put(splitedWorld, counter.getOrDefault(splitedWorld, 0)+1);
        }
